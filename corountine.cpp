...
public:
    ...
    /**
     * @brief 协程状态
     * @details 在sylar基础上进行状态简化，只定义三态转换关系，也就是协程要么正在运行(RUNNING)，
...
public:
    ...
    /**
     * @brief 将当前协程切到执行状态
     * @details 当前协程和正在运行的写成进行交换，前者状态变为RUNNING，后者状态变为READY
     */
    void resume();
...
public:
     ...
     * 且该协程为当前线程的主协程，其他协程都通过这个协程来调度，也就是说，其他协程结束时，
    ...
    : m_id(s_fiver_id++), m_cb(cb)
{
     ...
     * @attention 线程如果要创建协程，应当首先执行下Fiber::GetThis()操作，以初始化主函数协程
     */
    Fiber::ptr GetThis()
    {
        if (t_fiber)
